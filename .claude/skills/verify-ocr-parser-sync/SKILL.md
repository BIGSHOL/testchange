...
| File | Purpose |
|------|---------|
| `core/ocr_engine.py` | Claude Vision OCR 엔진 (프롬프트, JSON 파싱) |
...
**검사 5b:** config.py의 CLAUDE_MODEL 기본값이 유효한 Claude 모델인지 확인

```bash
grep "CLAUDE_MODEL" utils/config.py
```

**PASS:** 기본값이 `claude-haiku-4-5-20251001` 또는 유효한 Claude 모델 ID
