...
> 최종 갱신: 2026-06-10 (커밋 `389e974`). ✅ **소스 HEAD=`389e974`, 배포 exe=`389e974` 빌드 완료**
> (2026-06-10 재빌드·`배포용/` 배포, `--selftest` = `SELFTEST OK (v0.1.14)` + `GEMINI LIVE OK`,
> `config.json` 379B 보존). 경운중 폼 4건·표복구/폼측정/서수'제' 전부 배포 반영됨 → 추가 빌드 불필요.
...
> ✅ 위 §7 ✅ 수정들(content_parser·hwp_form_writer)은 **2026-06-10 `389e974` 빌드에 반영·배포 완료**.
