...
# 프라임(') 허용 — 대칭이동/접기의 상(像) 점 ``A'(7,4)``·``B'(-3,-1)`` 이 점 글자 로만화에서
# 빠져 통째 이탤릭이던 것(사용자 2026-08-07 렌더 지적; 같은 해설의 원래 점 ``A(4,7)`` 은 정자라
# 혼재). 도함수 ``F'(x)`` 는 좌표쌍(쉼표/수 리터럴) 조건과 기하 게이트에 걸려 미매치 — 안전.
...
# 괄호 그룹 — **한 단계 중첩 허용**. ``(f(x))^5`` 처럼 괄호base 안에 함수호출 괄호가 또 있는
# 경우(남산고 미적분 단답형4), 무중첩 ``\([^()]*\)`` 는 ``(f(x))`` 를 못 잡아 ``(`` + ``f(x)`` +
# ``)^`` 로 쪼개져 ^지수가 literal 캐럿으로 샜다(능인고 #18 무중첩 ``(1+h)^n`` 의 중첩 확장판).
# 한글 제외(텍스트 괄호 "(즉…)" 오인 방지). 단일-char vs 중첩그룹 분기라 백트래킹 선형.
_PAREN_GROUP = r'\((?:[^()가-힣]|\([^()가-힣]*\))*\)'
_MATH_ATOM = (r'(?:(?:[a-zA-Z0-9]+(?:\.[0-9]+)?|' + _PAREN_GROUP + r')'
              + _SUBSUP + r')+(?:\s*' + _PAREN_GROUP + r')?')
...
def _split_latex_commands(text: str) -> list[ContentBlock]:
    ...
    for _i, _ch in enumerate(rest):
        if _ch == "{":
            ...
            _ends.append(_i)
            break
        elif _depth == 0 and ("가" <= _ch <= "힣" or "ㄱ" <= _ch <= "ㆎ"):
            # ⚠️ 음절(가-힣)뿐 아니라 **호환 자모**(ㄱ-ㆎ)도 수식 종료 경계 — 보기 박스 항목
            # 라벨 ㄱ.ㄴ.ㄷ.ㄹ. 은 자모라, 음절만 검사하면 ``\lim…=\lim… ㄷ. x<-1`` 처럼
            # 라벨+뒤 수식조각이 통째 한 수식에 흡수돼 박스 줄바꿈(_BOX_BREAK_RE)이 깨진다
            # (대구여고 미적분 #8 보기, 2026-06-16 — 장산중 D2·새본리중 #17 자모 함정의 또다른
            # 경로). depth>0(``\boxed{가}``)은 보호. line 1686 의 ``[가-힣ㄱ-ㆎ]`` 원칙 동일.
            _b = _i
