...
작동(B 단계 연결 후):
  - 런타임에 core/ocr_engine._active_prompt() 가 EXAM_OCR_PROMPT 뒤에 이 파일의 내용을
    append 한다(주석 <!-- --> 과 빈 줄만 있으면 no-op).
  - scripts/ocr_eval/prompt_version._payload() 가 이 파일 내용을 서명에 포함 → 보강이 바뀌면
    prompt_signature 가 바뀌어 eval 캐시가 분리되고 A/B 가 성립한다.

규칙:
  1. 인스턴스 암기 금지 — '원본 그대로 옮겨라' 류의 **일반 규칙**으로만 군집해 적는다.
