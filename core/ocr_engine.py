...
4. 배점은 **score 필드에 인쇄된 표기 그대로 문자열로** — "[3.0점]" 이면 `"score": "3.0"`, "[4점]" 이면 `"score": "4"`, "[4.2점]" 이면 `"score": "4.2"`. ⚠️ **소수점 뒤 0 을 지우지 마세요**("3.0"→"3" 금지) — 같은 시험지에서도 [3.0점]과 [4점]이 함께 인쇄되고, 숫자로 바꾸면 원본 표기를 복원할 수 없습니다. **본문 contents 텍스트에는 [N점]을 절대 넣지 마세요** (중복 출력 원인). 단, "[총 9점]"처럼 소문항 합계 총점은 본문에 그대로 두세요.
...
- ⚠️ **셀 수치의 음수 부호(−)를 절대 놓치지 마세요** — 편차·좌표 표의 수치 앞 짧은
  가로선은 음수 부호입니다(스캔에서 흐려도 부호 하나로 답 전체가 밀립니다: −2≠2).
- ⚠️⚠️ **학생 필기(펜 동그라미·빗금·풀이 획)가 표 셀 위를 가로질러도, 그 밑에 인쇄된
  부호·숫자는 전부 살려 읽으세요.** "손글씨 배제" 규칙은 필기 **획 자체**를 값으로 오인하지
  말라는 뜻이지, 필기에 **가려진 인쇄 활자**를 버리라는 뜻이 아닙니다. 판별 앵커: 같은 표의
  다른 음수 셀과 **같은 폭·굵기·높이·수직 위치의 짧은 가로선**이 값 바로 앞에 있으면 그건
  필기가 아니라 인쇄된 음수 부호입니다(운암중 #21 실사고: 펜 원이 A 셀 −2 를 관통 → −
  소실 → x 값과 답이 통째로 밀림. 같은 행의 −8·−10 과 부호 활자가 동일했음).
- 편차(deviation) 표는 **모든 값이 숫자면 합이 0** 이어야 합니다 — 합이 0 이 아니면 부호를
  놓친 것이니 각 셀 앞 가로선을 다시 보세요(미지수 x 가 있으면 이 검산은 생략).
...
## 주의사항
- 배점이 있으면 score에 **인쇄된 표기 그대로 문자열**로 기록("3.0"·"4"·"4.2").
- 이미지에 문제가 여러 개 있으면 모두 추출.
- **이미지의 모든 텍스트를 빠짐없이 추출하세요.**
"""


# 독립 그림 박스 서술 — 크롭 검출이 kind="figure" 로 분리한 박스는 OCR 을 안 거쳐
# (안내문구 대체) 해설 AI 가 그림 정보를 전혀 못 받았다(왕선중 [문제 오류] 실측,
# 2026-08-09). 이 프롬프트로 그림만 텍스트 서술을 뽑아 **해설 입력에만** 넣는다
# (figure 블록은 파서·어댑터가 드롭하므로 렌더 무영향). 현재 소비자는 웹
# `/api/figure-desc`(sync-prompts 로 동기화) — exe 도입 시 워커에 연결한다.
FIGURE_DESC_PROMPT = r"""이 이미지는 수학 시험 문항에 딸린 그림입니다. 문제를 풀 사람이
그림 없이 텍스트만 보고도 풀 수 있도록, 그림의 정보를 서술하세요.

- **데이터가 답을 결정하는 그림은 값을 전부** 적으세요:
  · 산점도 = 모든 점의 (x,y) 좌표를 하나도 빠짐없이 나열(겹치는 점은 개수도).
    축 라벨·단위 포함. 예: "산점도(말하기 x, 쓰기 y): (2,3),(3,5),(5,6)×2,… 총 20명"
  · 히스토그램/도수분포다각형/막대 = 계급(항목)별 값 전부. 꺾은선 = 각 x 의 y 값 전부.
  · 줄기와 잎 = 줄기별 잎 전부.
  · 기하 도형 = 인쇄된 모든 치수·각도·점/변 라벨과 위치 관계
    (예: "원 O, 반지름 6, 현 AB=8, 중심에서 AB 에 수선 OH, 점 P 는 호 AB 위").
  · 함수 그래프 = 식·절편·꼭짓점·교점·점근선 등 표시된 값 전부.
- 단순 삽화·상황 그림이면 한두 줄 요약이면 됩니다.
- 산점도·좌표평면의 점은 **x 축 눈금을 왼쪽부터 열(column) 단위로 훑으며** 각 열의
  점을 위에서 아래로 세어 나열하세요(눈금선과 점의 정렬을 정확히 대조 — 한 점을 옆
  칸으로 옮겨 적으면 답이 바뀝니다). 나열이 끝나면 **그림을 다시 훑어 총 개수를 검산**
  하고, 목록의 개수와 다르면 빠뜨린 점을 찾아 보완하세요.
- ⚠️ **사진 스캔은 원근 기울기로 격자선이 휘어** 있습니다(위로 갈수록 열이 옆으로
  밀림 — 실측: 세로 투영으로 읽으면 한 칸씩 틀림). 점의 좌표는 화면상 수직 위치가
  아니라 **그 점이 놓인 격자선을 축 라벨까지 따라가서** 판정하세요.
- 문항 본문 발췌가 함께 주어지고 거기에 **데이터 총원**(예: "학생 20명")이 있으면,
  판독한 점 개수와 대조해 다르면 빠진/중복된 점을 찾아 재검토하세요.
- ⚠️ 스캔본에는 **학생 필기(연필 동그라미·체크·선)가 인쇄된 점·수치 위에 겹쳐** 있을 수
  있습니다. 필기 표시 자체는 무시하되, **필기에 가려지거나 동그라미가 쳐진 인쇄 점은
  반드시 포함**하세요(가려졌다고 빼면 개수가 틀립니다).
- 라벨 사이의 **연결 관계는 단정하지 마세요**: 어느 선이 어느 점을 잇는지 확실할 때만
  "선분 AB" 로 쓰고, 불확실하면 "점 A 근처에서 점 P 쪽으로 뻗는 선" 처럼 보이는
  대로만 서술하세요(틀린 연결 단정 하나가 문제 전체를 모순으로 만듭니다).
- **각 라벨(x°, y°, ∠표시)은 위치를 명시**하세요: 어느 꼭짓점에 있는지, 어느 두 선
  사이의 각인지(예: "x 는 두 현 AC·BD 의 교점 P 에서 D·C 쪽으로 열린 각"). 각이
  원주각인지 중심각인지 교점의 각인지가 풀이를 좌우합니다(운암중 #18: 교점의 각을
  원주각으로 오해해 40°≠70°).
- **음수 부호(−)를 절대 놓치지 마세요** — 표·편차·좌표의 수치 앞 짧은 가로선은 음수
  부호입니다(스캔에서 흐려져도 부호 하나로 답 전체가 밀립니다. 운암중 #21: −2→2).
- **실제로 그려진 선·도형만** 서술하세요. 그려져 있지 않은 도형을 "사각형 ABCD 가
  형성된다" 처럼 **추론해 만들어 내지 마세요** — 그 추론이 틀리면 푸는 사람이 잘못된
  성질(내접 등)을 전제하게 됩니다.
- **원이 있으면 중심의 위치 관계를 반드시** 서술하세요: 중심이 현·밑변의 **어느 쪽**에
  있는지(꼭짓점과 같은 쪽/반대쪽), 내접 도형이 큰 활꼴/작은 활꼴 어느 쪽에 몰려
  있는지. 텍스트 조건만으로 배치가 두 가지인 문제는 이 정보가 답을 가릅니다
  (운암중 서답형4: 꼭짓점 A 와 중심 O 가 현 BC 의 **반대쪽** → 꼭짓점-현 거리
  10−8=2 인데, 위치 서술이 없으면 10+8=18 로 풀어 2√10 ≠ 6√10).
- 산점도의 **(가)·(나) 같은 라벨 붙은 점은 좌표를 라벨과 짝지어 개별 명시**하세요
  (예: "(가)=(60,85), (나)=(105,105)"). 라벨 점을 지우거나 추가하는 선택지는 이
  좌표가 있어야만 판정됩니다(운암중 #15).
- 그림에 인쇄된 수치·라벨만 적으세요(추측·계산 금지). 수식은 LaTeX 로.
- 서술 **텍스트만** 출력하세요(머리말·설명·JSON 없이).
"""
