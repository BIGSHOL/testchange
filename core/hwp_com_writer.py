...
# 대화 화자 라벨(교사:/학생:/선생님:) — 대화 박스(제일중 서술형1)에서 각 발화를 새 줄로.
# OCR 이 화자 전환마다 \n 을 줘 블록은 이미 분리되나(_split_on_newlines 가 \n 드롭), 박스
# 렌더러엔 줄 경계 신호가 없어 대화가 통째 흘렀다(2026-06-26). 라벨은 출력(은닉 아님)+앞 줄바꿈.
_SPEAKER_LABEL = r"(?:(?<=\s)|^)(?:교사|학생|선생님|선생)\s*[:：]"
...
class HwpComWriter:
    ...
    def _write_score(self, score, leading_space: bool = True) -> None:
        # 배점 숫자도 수식 객체로(A8). "[" "점]"는 텍스트.
        self.s.text(" [" if leading_space else "[")
        self.s.equation(score_str(score))
        self.s.text("점]")
