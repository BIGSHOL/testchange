...
| `manage-skills` | 세션 변경사항을 분석하고, 검증 스킬을 생성/업데이트하며, AGENTS.md를 관리합니다 |
...
`.Codex/settings.json` 의 PostToolUse 훅이 위 파일(+`latex_to_hwpeq.py`) 편집 시마다
...
2. **배점** — 객관식: 발문 끝 인라인 `[N점]`. **서술형(독립·소문항 공통): 발문 끝 인라인 우선,
   ...
   `KeyIndicator()[5]` 줄 비교. 사용자 2026-06-16 — 과거 force_break[항상 줄바꿈] 폐기). **소문항
   부모의 총점은 발문 본문의 `[총 N점]` 을 떼어내(`_split_trailing_score`) 인라인 우선/줄넘침 시 우측정렬**.
...
8. **부정 선택문 부정어 = 볼드+밑줄**(2026-06-16) — "옳지 **않은** 것"·"**아닌** 것"·"**틀린** 것"
   의 부정어를 강조. parse 단계 `content_parser._emphasize_negation`(부정어 뒤 '것'일 때만
