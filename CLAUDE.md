...
### ⚠️ 인접 기존 결함 2건 (이번 수정과 무관 — 사용자 판단 대기)
- **합성함수 ``f \circ g`` → ``f ° g``**: SYMBOL_MAP ``\circ``→``°`` 가 위첨자 아닌 단독
  ``\circ`` 까지 도(°)로 만든다. 정상은 ∘(U+2218). OCR 이 각도를 ``90\circ``(위첨자 없이)로
  주는 경우와 트레이드오프라 미수정.
- **``LL`` 라벨 → ``"L rm`L"``**: `_romanize_units` 의 ``_VAR_UNIT_RE``(변수 뒤 L=리터)가
  ``LL`` 둘째 L 을 단위로 오인. HEAD 에서도 동일(무회귀), corpus 에 해당 라벨 없음.
