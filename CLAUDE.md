...
## ⭐ 대륜고 공수2 웹 변환 검수 — 답지 페이지 유령 문항 연쇄 (2026-08-20, 사용자 보고)

`(원본+답).pdf` 웹 변환물에서 사용자가 지적한 6건. **한 가지 근본원인이 여러 증상을
만들었다** — 문제지 뒤 답지 2쪽이 크롭·OCR 대상이 되어 봉투에 **28문항**(진짜 20 +
유령 8)이 실렸다. 진단은 Supabase `api_cache`(OCR·해설 원본)+`conversion_logs` 를 꺼내
**로컬 재현 렌더**로 확정했다(API 0원).

### ⭐⭐ 답지 페이지가 문항으로 — 라벨 재부여까지 죽인다
- 유령 8개가 본문에 채점기준을 문항으로 찍고, **서답형을 7→15 로 부풀려**
  `_renumber_essay_labels` 의 오손상 방지 가드(`라벨 33 ≠ 2×15`)를 발동 → **정답면 라벨
  재부여가 통째로 생략** → 폼 grow 잔재 `[서술형 5]` 셋이 그대로 출하됐다(사용자 지적 1).
- 수정 ①: 크롭 프롬프트에 **`class="answer"`** 추가(정답표·채점기준·모범답안 페이지는
  번호가 있어도 문제가 아니다). 소비자(GUI·웹 App.tsx)가 그 박스를 OCR 대상에서 뺀다 —
  웹은 여기서 빼야 **호출 요금**도 안 나간다.
- 수정 ②(모델 무관 안전망): `models.exam_document.drop_answer_key_questions` —
  **선택지 없음 + 배점 없음 + label_type 없음 + 번호가 앞선 서답형과 중복 + 본문에 라벨
  없음** 을 전부 만족하는 **뒤쪽 연속 구간**만 걷어낸다(답지는 항상 뒤에 온다). 빈 문항은
  위치 무관 제거. **절반 이상을 답지로 판정하면 판정을 통째로 버린다**(문제지 증발 방지).
  웹=파싱 전 봉투(`convert_cli`), exe=파싱 후 문서(`drop_answer_key_pages`).
  전 corpus 4,202문항 스캔 **0편 영향**(무회귀).

### ⭐ 라벨 재부여는 **본문/정답면 각각** — 한쪽 중복이 다른 쪽을 죽이면 안 된다
- `_renumber_essay_labels` 가 전체 라벨 수 하나로 판단(`≠ 2×N` 이면 통째 생략)해, 본문에
  OCR 라벨 잔재가 **하나만** 남아도 정답면까지 포기했다. → `<hp:endNote>` 안/밖으로
  **나눠 각 쪽을 독립 재부여**(미주가 없는 문서는 종전 문서순 쌍 폴백).
- 같은 증상의 다른 갈래: 라벨 정규식 3개가 `서[답술]형` 이라 **단답형을 빠뜨렸다** →
  `[단답형 5]` 가 본문에 남아 폼 라벨과 이중 표기되고, 번호가 수식이 아닌 평문으로 찍혀
  후처리가 그 문항을 못 셌다. 셋 다 `(?:서[답술]형|단답형)` 으로 교정.
- 유형 미상 문항(크롭 OCR 실패)은 **같은 시험지의 다른 서답형 유형**을 따른다(균일할 때).

### ⭐⭐ 배점은 **인쇄 표기를 보존**한다 — 값으로는 절대 구분 못 한다
- 첫 시도는 "문서에 소수 배점이 있으면 정수도 `N.0`" 휴리스틱이었는데 **원본 실측으로
  반증**됐다: 같은 소수 사다리인데 대륜고 #8 = `[3.0점]`, 대진고 #3 = `[4점]`.
  전 corpus 108편이 바뀌는 회귀였다(적대 검증에서 폐기).
- 확정: **표기를 문자열로 보존**한다. `score_str` 은 문자열이면 그대로 내보내고, 숫자면
  종전 동작. 파서는 본문에서 캡처한 `3.0` 처럼 **소수점 뒤가 0** 인 표기만 문자열로 남긴다
  (나머지는 숫자 — 소비자 타입 유지). OCR 프롬프트도 "인쇄된 표기 그대로 문자열"로 개정.
  ⚠️ **이미 캐시된 OCR JSON 은 표기를 잃은 상태**라 재OCR 해야 고쳐진다.

### 그 밖에
- **`\sim` 문맥 분기**: 이항이면 닮음 ∽(중등 기하), **단항이면 명제의 부정 ∼**(U+223C).
  앞 토큰이 피연산자(영숫자·`}`·`)`)면 이항, 비었거나 관계·논리 **명령**(`\rightarrow`)
  이면 단항. corpus 14건 전수 확인(닮음 13 정상 유지). 과이스케이프 `\\sim` 은 종전(∽).
- **step 라벨 분리형**: `step 1)` 처럼 띄어 오면 숫자가 수식으로 떨어져 정규화가 건너뛰고
  한 문서에 `rm step1)`(정상)과 날것이 섞였다 → 선두 `TEXT "step" + EQ "N"` 을 합쳐 재투입.
- **해설 잘림 방어**(웹): `finish_reason=="length"` 면 **캐시하지 않고** 응답에
  `truncated:true` 를 실어 사용자에게 경고한다(서버 로그에만 남으면 아무도 못 본다).
- 사용자 지적 중 **해설 문장 미완성·논리 단계 누락은 엔진 결함으로 재현되지 않았다** —
  캐시된 DeepSeek 해설은 완전하고 `finish_reason` 은 전부 `stop`, 엔진 격리 테스트도
  전문을 보존한다. 렌더 파일의 해당 미주 텍스트는 캐시·로그 어디에도 없는 제3의 텍스트다.
...
``서답형`` 으로 모았는데, ① 같은 시험지에서 OCR 이 한 문항만 다르게 읽으면
``[서술형 1~3]`` 사이에 ``[서답형 4]`` 가 끼고(대륜고 공수2 실사고) ② 시험지마다 용어가
갈려 출력이 들쭉날쭉했다. 이제 **항상 한 방향**으로 모은다.

⚠️ **``단답형`` 은 건드리지 않는다** — 서술형과 다른 유형이고 원본도 그렇게 인쇄한다
(능인고 수1·정화중처럼 두 유형이 함께 있는 시험지가 실재한다).

라벨은 **여러 경로**로 출력에 들어가므로 한 군데만 고치면 샌다:
- **파서**(`content_parser._normalize_essay_label_type`): 본문 TEXT 의 낱말 ``서답형`` 을
  통째 치환 + `label_type` 필드(소문항 포함). ⚠️ 라벨은 인라인 수식 분리로
  ``TEXT"[서답형 " + EQ"4" + TEXT"] …"`` 로 **쪼개져 들어오는 일이 흔해**, ``[서답형 N]``
  패턴만 보면 쪼개진 쪽을 통째로 놓친다(그래서 낱말 치환).
- **폼 writer**(`_norm_essay_word`): 라벨 조립 4형태(괄호/분리형/비괄호/한블록)와 폴백
  전부에 적용. 유형 미상 문항의 기본값도 ``서술형``.
- **저장후 XML 안전망**(`_force_essay_word`): 폼이 미리 구워 둔 정답면 라벨·grow 슬롯
  복사본까지 ``[서답형``→``[서술형``. `_sync_essay_label_word` 는 **균일 시험지에서만**
  돌아 서술형+단답형 혼합을 못 덮으므로 이 패스는 **항상** 돈다(relaunder 루프에서도).
- 회귀: `test_render_fixes` essay-word-unify(단답형 보존 포함).

## ⭐ 웹 그림이 안내문구로 떨어진 두 원인 + 도우미 config 정리 (2026-08-20, 사용자 보고)

대륜고 공수2 웹 변환물에서 **서술형4 발문 증발**과 **그림 미작도**가 함께 보고됐다.
Supabase 캐시(`api_cache`)로 **로컬 재현**해(API 0원) 원인을 갈랐다 — 둘은 별개다.

### ⭐⭐ 서술형4 발문 증발 = **크롭 y 드리프트**(OCR 은 무결)
- p4 서답형4 검출 박스가 ``y 0.544~0.580`` 인데 실제 문항은 ``y≈0.482~0.545`` —
  **한 블록 아래로 밀려** 발문을 통째로 놓치고 꼬리만 걸렸다. OCR 결과는
  ``{"score":"13","number":4,"contents":[{"text":"다.)"},{figure}]}`` — 자기가 받은
  크롭에 충실했다(``(단, a는 실수이"다.) [13점]"`` 의 마지막 조각).
- 같은 페이지의 다른 두 박스는 정확했고, **직전 런은 같은 문항을 ``0.579~0.644`` 로**
  잡았다 → 프롬프트로 막을 성질이 아니다(비전 모델 y 좌표가 시도마다 흔들린다).
- 수정 = **결과가 비정상일 때만 되돌린다**(웹 `App.tsx`): 문항 크롭 OCR 결과의 본문이
  25자 미만이면(배점·그림만 남은 상태) **위로 0.10 넓혀 1회 재판독**하고, 본문이 늘면
  채택한다. 정상 크롭에는 호출이 늘지 않는다. 크롭 교체 시 이후 그림 bbox 기준도 함께
  바꾼다(같은 좌표계를 써야 한다).

### ⭐⭐ 그림 미작도 = **품질 게이트 오탐**(작도는 멀쩡했다)
- 모델이 습관적으로 넣은 배경용 ``<rect width="400" height="280" fill="none"/>`` 하나가
  ``<rect> has neither a visible fill nor stroke`` 로 걸려 **멀쩡한 무리함수 그래프가
  통째 반려**됐다(로컬 `assess_svg` 재현으로 확정). 안 보이는 사각형은 래스터라이즈해도
  픽셀이 안 남는데 그 존재만으로 그림을 버린 것이다.
- 수정 = `_looks_like_background_rect` 가 **캔버스 전면 rect 를 fill 무관 배경으로** 본다
  (기존 white 에 none/transparent 추가). ⚠️ **넓게 풀면 안 된다** — 이 게이트는 "안 보이는
  요소로 내용을 있는 척하는" 생성물을 막는 장치다. 처음엔 '안 보이는 요소' 전부를 경고로
  강등했다가 `test_figure_generator_quality` 의 **투명 라벨·투명 도형 반려** 케이스가
  깨졌다(그 테스트가 옳다). 전면 rect 는 특정 도형을 흉내 낼 수 없어 예외가 안전하다.
- 수정 = **크롭을 항상 함께 보낸다**(웹 `App.tsx`). 종전엔 "작도가 있으면 payload 를
  아끼려고 크롭 생략" 이었는데, 게이트가 반려하면 **폴백이 하나도 안 남아** 안내문구로
  떨어졌다. 커넥터는 로컬이라 몇 MB 는 싸다 — 그림을 잃는 것보다 훨씬.
- ⚠️ `spec` 이 항상 null 인 것은 **정상**이다 — SVG_RULES 가 곡선 그래프에는 FigureSpec
  생략을 지시한다(라벨 자동배치는 원·점·선분 도형용). 이 시험지는 전부 함수 그래프였다.

### 도형 작도 로그(사용자 요구)
- `convert_cli` 가 작도 경로를 집계해 stderr + 진단에 싣는다:
  ``[convert] 도형 엔진 — 작도 2개(스펙 0 · SVG 2) · 원본 크롭 0개 · 안내문구 0개 ·
  게이트 반려 0건``. 웹은 이 진단으로 ``그림 — 도형 엔진 작도 2개`` 한 줄을 띄운다
  (안내문구가 있으면 warn).
- ⚠️ resvg 가 없어 PNG 래스터화가 실패한 것을 "게이트 탈락" 으로 찍고 있었다 → 문구 분리
  (원인을 품질 게이트에서 찾게 만든다).

### 사용자 화면 로그 정리
- 마지막에 찍히던 ``렌더 경로 — {"questions":20,"form":…,"hwp_version":…}`` 원문 JSON 은
  **화면에서 제거**하고(사용자: "사용자가 보기에 필요없을듯") 서버 기록(detail)에만 남긴다.
  화면에는 ``서식 — [대수회]…`` 한 줄, 폼 폴백이면 경고 한 줄.
- **favicon 없음 → `/favicon.ico` 404** 가 페이지마다 찍혔다 → data: URI 파비콘.
- **도우미 미실행 시 `/health` 8초 폴링**이 `ERR_CONNECTION_REFUSED` 로 콘솔을 덮어
  진짜 오류를 묻었다 → 실패가 이어지면 8s→20s→45s 로 백오프, **성공하거나 탭에 돌아오면
  즉시 복귀**(도우미를 켠 직후 감지 속도는 유지).

### ⭐ 도우미는 로컬 `config.json` 을 만들지 않는다
- 도우미(agent.exe/커넥터)가 이 파일에서 **실제로 읽는 값은 `EQ_WATERMARK` 하나**다 —
  키·모델·QC·DeepSeek 설정은 전부 서버(Vercel env)에 있고 도우미는 OCR·해설을 안 한다.
  그런데 `utils.config` 가 파일이 없으면 기본값 27개를 써 놓아, "여기에 API 키를 넣어야
  할 것처럼 보이는" 파일이 도우미 폴더·배포 zip 에 생겼다(사용자 지적). 빌드 폴더에
  **진짜 키가 든** config.json 이 있으면 그대로 배포될 위험도 있었다.
- 방어 **두 겹**(하나면 조용히 되돌아간다): ① 진입점(`agent.py`·`server/connector.py`·
  `server/convert_cli.py`)이 **core/utils import 전에** ``MATHGEN_CONFIG_READONLY=1`` 을
  세운다(환경변수라 자식 워커까지 따라온다) ② `scripts/build_agent.py` 가 번들·zip 에
  config.json 이 있거나 키처럼 생긴 값이 보이면 **빌드 실패**.
- GUI exe 는 종전대로 최초 실행에서 config.json 을 만든다(사용자가 키를 넣는 파일).
- ⚠️ `gate:connver`(웹)가 두 군데서 고장나 있었다: 접미사 케이스가 MIN 상수에 매달려
  MIN 을 올리자 FAIL, 엔진 경로가 `D:\시험지 한글화` 하드코딩이라 **가장 중요한 대조
  (웹 요구 ≤ 커넥터 실제)가 영영 SKIP**. 둘 다 수정.

- 회귀: `tests/test_figure_gate_warnings.py`(전면 rect 통과 / 부분 투명·투명 라벨 반려) ·
  `tests/test_helper_config.py`(읽기 전용·진입점 순서·빌드 게이트) ·
  `test_connector_contract` I(⑤ SVG 반려 + 크롭 동봉 → 원본 그림, 집계 stats).
...
## ⭐ 단위 이탤릭·mL 쪼개짐 + 워터마크가 라벨 후처리를 죽인 회귀 (2026-08-11, 세션 변환 검수)

운암중 중3 25-2-기말 세션 변환(구독 비전 OCR → 중3 빨강폼)에서 잡은 **B형 3건**. 전부
결정적 수정 + corpus 전수 OLD/NEW(27,812 수식) 스캔 **변경 49건 전부 개선 방향, 회귀 0**.
회귀 `test_render_fixes` O9(단위)·H2(표식 라벨).

### ⭐ `\mathrm{m}`·`\mathrm{g}` 단위가 이탤릭 — rm 자기 피연산자를 번짐으로 오인
- 증상: ``20\mathrm{m}`` → ``20 rm it {m}`` 로 나가 **미터 단위 m 이 기울어** 렌더됐다.
  ``\mathrm{g}``(그램)도 동일. 다문자 단위(``cm``·``km``)는 멀쩡해 한 문서에 정자/이탤릭 혼재.
- 원인: `_stop_roman_bleed`(2026-07-27 왕선중 #9 — ``rm`` 뒤 **단일 소문자 변수**를 번짐
  피해자로 보고 ``it {}`` 로 감싸 이탤릭 복원)가 bare ``rm m`` 에서 **rm 자신의 피연산자**와
  뒤따르는 변수를 구별하지 못했다. 다문자·대문자는 스캐너(`[a-z]` 단일 글자) 대상이 아니라
  살아남아 잠복했다.
- 수정: `latex_to_hwpeq._mathrm_repl` 이 **단일 소문자 내용을 중괄호로** 감싼다(``rm {m}``) —
  bleed 스캐너의 ``(?<![A-Za-z0-9`{])`` lookbehind 가 걸러 건너뛴다. `_backtick_rm_units` 도
  중괄호 형태를 받아 ``20\mathrm{g}`` → ``20 rm`g``(bare ``20g`` 와 동일 간격)로 통일.
- 영향: corpus 12개교 48건이 **이탤릭 미터/그램으로 출하돼 있던 숨은 결함**(재렌더로 자동 정정).

### ⭐ `\mathrm{mL}` → `rm m rm`L`("m 리터") 쪼개짐
- 이미 로만화된 런 안에서 `_VAR_UNIT_RE` 가 앞글자 ``m`` 을 변수로, 끝 ``L`` 을 리터로
  재해석했다(**단위가 L 로 끝나는** mL·dL·kL 공통). ``rm {AL}`` 라벨 가드(2026-08-07)와 같은
  계열 — lookbehind 에 ``(?<!rm )``·``(?<!rm`)`` 추가. bare ``100mL`` 도 함께 정상화.

### ⭐⭐ 수식 워터마크가 서답형 라벨 재부여를 **조용히 비활성**시킴 (전역 회귀)
- 증상: 정답면 마지막 서답형 라벨이 ``[서답형 5]``(6이어야) — 상인고 #25 와 같은 grow 잔재인데
  이를 고치는 `_renumber_essay_labels` 가 "라벨 수 불일치(6 ≠ 2×6) — 재부여 생략" 으로 빠졌다.
- 원인: **소유권 표식**(`eq_watermark`, 2026-08-10 도입 — 스크립트 끝에 ``\nfrom {…}``)이 우리가
  쓴 **본문 라벨 수식에만** 붙는다(정답면 라벨은 폼 native 라 무표식). 라벨 정규식이
  ``<hp:script>(\d+)</hp:script>`` 로 **숫자 직후 닫는 태그**를 요구해 표식 달린 본문 라벨 6개를
  통째로 놓쳤고, 절반만 세어져 2×N 오손상 가드가 발동했다.
- 수정: `_ESSAY_LABEL_NUM_RE`·`_ESSAY_NUM_EQ_RE` 에 선택적 표식 tail(`_EQ_MARK_TAIL`) 허용.
- ⚠️ **교훈: 수식 스크립트 내용을 건드리는 기능(워터마크 등)을 넣으면 `<hp:script>` 를 읽는
  후처리 정규식을 전부 재점검**해야 한다. 이 회귀는 렌더가 안 깨지고 lint 도 PASS 라
  **2026-08-10 이후 모든 폼 렌더에서 라벨 후처리가 죽은 채** 출하됐다.

### 이상문자열 게이트 보강 — 완성 음절 혼입은 기존 게이트가 못 잡는다
- 렌더 중 키 입력이 숨김 COM 문서로 새는 현상(강동중 이래 반복)이 이번엔 ``점]브  ``·``애려``
  처럼 **완성 음절**로 들어와, corpus_lint 의 자모 검사(`_LONE_JAMO_RE`)·단독 런 휴리스틱을
  전부 통과했다. 확실한 검출법 = **한글 문자집합 대조**: 출력 `<hp:t>` 의 한글이
  (OCR JSON ∪ 폼 상용구 ∪ header_values)에 없는 음절이면 혼입.
  ⚠️ 폼 정답면의 관리 필드 ``워드:t``·``오검:t``·``완료:t`` 는 템플릿 내용이니 화이트리스트에.
  혼입은 **비결정**이라 게이트 통과할 때까지 재렌더하면 된다(이번 1~4회).
...
## ⭐⭐ 렌더는 성공했는데 500 — 산출물 확장자 폴백 유실 (2026-08-10, 커넥터 1.1.2)

한글 **2024** PC에서 웹 변환이 끝까지 가다 마지막에 500. 사용자 로그로 사슬 확정:

```
폼 채움 실패 (-2147417851 RPC_E_SERVERFAULT)   ← HWP 프로세스가 죽음
기본 서식 — 정답·해설 포함                      ← 렌더는 **성공**
HWP Quit 실패(고아 프로세스 가능)
최종 .hwp 저장 실패 — .hwpx 로 반환             ← 죽은 HWP라 굽기 실패(writer 는 정상 폴백)
렌더는 끝났으나 출력 파일이 없습니다             ← 요청한 .hwp 만 확인 → exit 3 → 500
```

- **핵심**: 파일은 멀쩡히 만들어졌는데 **확장자가 다르다는 이유로 버려졌다**. 합의 #12로
  최종 산출물을 `.hwp` 로 바꾼 뒤 생긴 구멍 — writer 는 실패 시 `.hwpx` 를 돌려주는데
  `convert_cli`/커넥터가 그 반환값을 안 봤다. `resolve_output` + 커넥터 `(bytes, 확장자)`
  반환으로 전달. `.hwpx` 는 폼 바탕쪽 2단 세로선만 빠지고 내용은 온전하다.
- ⚠️⚠️ **부모가 산출물을 추측하면 안 된다**(적대리뷰가 잡은 실결함): HWP 가 `SaveAs`
  **도중** 죽으면 **잘린 `out.hwp` 가 남는다**. 부모가 요청 확장자를 먼저 집으면 그
  깨진 파일을 성공으로 내보낸다(500 보다 나쁘다 — 안 열리는 파일 + 로그엔 무표시).
  자식이 `_record_output` 으로 실제 파일명을 진단에 박고 부모는 그걸 **최우선**으로 믿는다.
- **폼 채움 2회 시도**: COM 크래시는 비결정적이라 고아 정리 후 재시도하면 폼을 살릴 수
  있다. ⚠️ 단 **첫 크래시가 늦게 나면(`_FORM_RETRY_BUDGET_S`=90s) 재시도 생략** — 늦은
  크래시에 재시도하면 [폼 2회 + 기본 서식]이 커넥터 타임아웃(360s)을 넘겨 **오히려
  결과가 통째로 없어진다**(재시도를 넣으며 만든 회귀를 적대리뷰가 지적).
- **고아 정리는 COM 크래시일 때만**(`_is_com_crash`): 변환이 분 단위라 PID 차집합에
  **사용자가 그 사이 띄운 한글**이 섞인다 — 무조건 죽이면 남의 미저장 문서를 날린다.
  일반 예외는 세션 컨텍스트매니저가 Quit 하므로 고아가 안 남는다.
- `hwp_pids`/`reap_hwp` 는 `core.hwp_com` 단일 출처(커넥터가 재사용). **이미지명 대조**
  (필터가 안 먹으면 엉뚱한 PID 를 죽인다) + **`taskkill` rc==0 만 집계**(하지도 않은
  청소를 주장하면 남은 고아를 가린다).
- **진단에 `hwp_version` 을 싣는다**(레지스트리만 — 한글 안 켬). ⚠️ 한글은 32비트 COM
  서버라 CLSID 가 **WOW6432Node 뷰**에 있다(64비트 파이썬 기본 뷰로 읽으면 조용히 빈 값).
  `/health` 에도 노출 — 버전 의존 크래시인지 가를 유일한 근거였다.
- 회귀: `test_connector_contract` J(확장자 폴백 8)·K(프로세스 정리 6).
- ⚠️ **미해결**: 그 PC 에서 **왜** 폼 채움이 크래시했는지는 재현 불가(개발 PC 한글 2020
  정상). 재변환은 정상 통과했으므로 **비결정적 크래시**였을 가능성이 높다(재시도로 회복).
  재발하면 진단의 `hwp_version`·`form_fallback_error` 로 추적할 것.
...
- **구현**: `core/eq_watermark.py`(`stamp`/`is_stampable`/`strip_mark`), config
  **`EQ_WATERMARK`**. 사용자 2026-08-10 "누가 지워도 상관없다 — 그 많은 수식을 하나하나
  다 지울 정도면 인정하고 냅두면 된다" → **기본 켬**, 문구는 `수식 변환기로 변환된
  수식입니다.` 고정(빈 문자열로 두면 OFF). 실측: 오성중 정답면 8쪽 **픽셀 차이 0**,
  우리가 만든 수식 417/417 표식(빠진 4개는 폼 템플릿 자체의 `[서답형 N]` 라벨 번호).
  적용 지점은 수식을 쓰는 두 곳 —
...
## ⭐ 웹 '폼 없이 2단' 모드 — 테스트 코드 TEST2020 (2026-08-12, 사용자 지시)

대수회 폼(슬롯 채우기) 대신 **학교 기출 시험지 양식 2단 바탕 위에 본문만 흘려 쓰는**
경로. 사용자 요구: "지정한 테스트코드로 변환할 때는 폼지에 작성 안 하고 그냥 OCR→HWP,
빈 새 문서에 **그냥 단순하게 2단**".

- **계약은 payload 한 필드** — 웹이 `{"layout":"plain2"}`(또는 `noForm:true`)를 실어
  보내면 `convert_cli.wants_plain_layout` 이 폼 매칭을 **건너뛴다**(`form_path=None`).
  플래그가 없으면 종전과 **바이트 동일** 경로(파일명 규칙 → 대수회 폼) — 회귀 0.
- **켜는 쪽**: 웹 `App.tsx` 의 `PLAIN2COL_CODES`(현재 `TEST2020`). ⚠️ 이 코드는 Vercel
  `INVITE_CODES` 에도 있어야 한다(`TEST2020:100`) — 없으면 /api/verify 가 먼저 403.
- **바탕 템플릿** `forms/plain2col/기본2단.hwp` — 사용자 양식(.hwp)에서 제작
  (`scripts/make_plain2col_form.py`). ⚠️ `forms/` **하위 폴더**에 두는 이유: `list_forms()`
  가 `*.hwp` 를 비재귀로 훑으므로 GUI 대수회 폼 드롭다운에 안 섞인다. 없으면 빈 새 문서
  + COM 간단 머리말로 폴백(agent.spec 빌드 게이트가 누락을 차단).
- **머리말은 토큰** `{{제목}} {{과목}}` — 파일명 규칙에서 `_plain_header_meta` 가
  "강동중 1학년 1학기 기말고사 / 수학" 을 조립해 `_fill_tokens` 가 치환. 규칙 미일치면
  빈칸(차단하지 않는다 — 2026-06-16 합의와 같은 태도).

### 사람이 만든 양식을 템플릿으로 쓸 때의 함정 (실측 2026-08-12)
1. **`SelectAll`+`Delete` 는 머리말까지 지운다** — 본문만 비우려면 `MoveDocBegin` +
   `MoveSelDocEnd` + `Delete`. (첫 시도에서 머리말이 통째로 사라진 변환물이 나왔다.)
2. **개요 번호(문단 자동번호) 서식이 남으면 이중번호** — 우리 문항번호와 겹쳐
   `1. 1. 곱셈 기호…`, 선택지 앞에도 `2. ①`. 본문 단락이 참조하는 paraPr 의
   `<hh:heading type="OUTLINE">` 을 NONE 으로(제작 스크립트가 처리, Ctrl+3/4 스타일은 보존).
3. **`use_endnote=False` 필수** — 미주로 두면 문서 끝에 **빈 미주 목록**(`1. 2. 3. …`)이
   인쇄된다. 대수회 폼은 그 미주가 답안칸이라 필요하지만 평문 2단엔 쓸 자리가 없다.
4. ⚠️⚠️ **템플릿 제작 중 타이핑 혼입이 템플릿에 구워진다** — 실측 2회 연속으로
   `서 ㄱ`·`ㅂ게 유` 가 본문에 박혔고, **재렌더해도 그대로**라(템플릿이 원인) 코드 결함으로
   오인하기 딱 좋다. 제작 스크립트가 [XML 정리 → 굽기 → **결과 파일** 재검사]를 깨끗해질
   때까지 3회 반복한다. 검사 대상이 '고친 XML' 이면 굽는 세션의 혼입을 놓친다.
- 한계: 2단 흐름 배치라 **문항이 단 경계에서 쪼개진다**(대수회 폼은 슬롯 높이를 실측해
...
### 강북고 공수2 변환물 검수 — 2단 여백·표 폭·중복 박스·빈 페이지 (2026-08-12, 사용자)
사용자가 웹 변환물(TEST2020 = 2단 경로)을 검수해 지적한 것 전부. 원본 PDF 대조로 원인 확정.

1. **여백** — 좌우 20mm 는 A4 2단에서 칼럼 81mm. 긴 수식 줄이 넘쳐 낱말 간격이 벌어지고
   배점이 `[2.7` / `점]` 로 갈라졌다. 사용자 지정값(좌우 10·위 5·아래 10·머리/꼬리 15)으로
   템플릿을 다시 구워 **칼럼 91mm**(제작기 `scripts/make_plain2col_form.py` 의 `MARGIN_MM`).
   참고: 대수회 폼은 **B4(257mm)** 라 칼럼 104.5mm — A4 에서는 91mm 가 한계다.
2. ⭐ **표 폭이 지면과 무관하게 계산됐다** — `_col_width_2col` 은 *A4 + 우리가 지정한 여백*
   가정인데, 폼/템플릿 위에 쓸 때는 용지·여백이 **폼 파일에서** 온다. 그래서 칼럼 91mm 인데
   보기 박스를 77mm 로 만들어 `<보기` / `>` 라벨이 접혔다. → `_section_col_width(section_xml)`
   로 **저장된 문서에서 직접 잰다**(`_inject_bogi_form`·`_fit_wide_tables_2col` 둘 다).
   `_scale_bogi_box_width` 도 상수 base 가 아니라 **그 표의 현재 폭** 기준(이중 적용 방지).
3. ⭐ **소문항을 되풀이하는 박스**(`content_parser._drop_subquestion_duplicate_boxes`) —
   원본엔 박스가 없는데 웹 OCR 이 (물음1)/(물음2)를 `<상자>` 로 인코딩하고 **두 번째 박스가
   발문 후반부까지 삼켜**, 같은 문장이 박스와 소문항에 두 번 인쇄되고 발문이 `…원과` 에서
   끊겼다. 박스 내용이 소문항 본문으로 **시작**하면 그만큼 잘라내고, 남는 게 없으면 박스를
   드롭·남으면 **박스 머리만 떼어 발문 연속으로** 되돌린다(내용 손실 0). 게이트: 소문항 보유
   + 박스 머리 블록 + 한글 12자 이상 접두 일치. corpus 4,161문항 스캔 **변경 0건**(무회귀).
4. ⭐ **정답면 앞 빈 페이지** — 마지막 문항 뒤 빈 줄 하나가 다음 쪽으로 넘어가고 정답면
   쪽나눔이 그 뒤에 걸려 **통째 빈 페이지**(문제 3쪽 → 빈 4쪽 → 정답 5쪽). `_write_question
   (trailing_gap=)` 으로 마지막 문항 + **그 마지막 소문항까지** 빈 줄을 생략.
- 회귀: `test_content_parser` KB1(중복 박스 제거 + 진짜 조건 박스 보존) ·
  `test_render_fixes` no-trailing-gap. 실렌더: 보기 박스 77.2→89.0mm(칼럼 91), 빈 페이지 0.
## ⭐ 그림 축 등방·분수 조판 + `$` 분리 세그먼트 재투입 (2026-08-18, 사용자 렌더 검수)

대진고 공수2 세션 변환물 검수에서 나온 3건. 전부 결정적 수정 + 전수 스캔.

### 그림 축 단위는 등방(ux == uy) — 비등방이면 곡선이 눌려 원본보다 넓다
- 증상: q12(30:22, x 1.36배)·q16(58:30, **x 1.93배**)이라 포물선이 가로로 늘어나고 직선
  기울기도 실제(56°)보다 완만. 실측 q16 67×41mm(W/H 1.64) vs 원본 72×64(1.14).
- 수정: 등방 재작도 + 삽입 폭 축소(q12 58→54mm, q16 72→60mm) → q12 46×40 · q16 55×61mm
  (원본보다 좁고 비례 일치). **작도 스크립트를 새로 쓸 때 축 단위를 다르게 잡지 말 것** —
  직각 표시·기울기·닮음이 전부 거짓말이 된다.

### 그림 안 분수 = `core.figure_svg.frac_label`(변수 이탤릭·숫자 정자)
- 증상: q16 `g(x)=2/a` 의 **분모 a 가 정자**. 시험지 스크립트가 분수 헬퍼를 자체로 갖고
  분자·분모를 `txt()` 로만 찍었다(복붙하면 시험지마다 재발).
- 수정: 엔진에 `frac_label`(+`runs_text`/`auto_runs`) — 라틴 문자는 이탤릭, 숫자·기호는 정자.
  `auto_runs` 입구에서 이중 escape 도 막는다(쪼갠 뒤엔 런 단위 검사가 엔티티를 못 본다).
- 회귀: `test_figure_svg_gate` 분수 조판 6케이스.

### ⭐⭐ `$` 분리 뒤 **맨 세그먼트도 수식 분리에 재투입** — 해설/본문 표기 불일치
- 증상: 서답형4 해설 step2 `삼각형 OAB의 넓이는` 의 OAB 가 **평문**인데, 같은 문장이
  본문에선 `rm OAB` 수식 객체(합의 #6). 사용자 지적.
- 원인: `_parse_content_block` 의 `$` 분기가 **백슬래시가 있는 세그먼트만** 재투입해서,
  `$` 밖의 맨 ASCII 수식(기하 이름·숫자)이 통째 TEXT 로 남았다. `__밑줄__` 분기가 수식
  추출을 억제하던 것(월서중 #14)과 **같은 계열의 구멍** — 분기가 파이프라인을 끊는다.
- 수정: `$` 분리 후 TEXT 세그먼트를 **전부** 재투입(밑줄 run 제외. `$` 가 소비돼 무한재귀 없음).
  덤으로 `_CIRCLE_NAME_RE` 에 `중심 O` 추가 — 단독 '중심'은 정규분포('평균을 중심으로')
  충돌로 키워드가 아니지만, **뒤에 바로 대문자 라벨**이 오면 원의 중심이라 로만(`원 O` 와 동형).
- 영향(전수 스캔): 해설·정답 113편 중 **29편**, 본문 4,202문항 중 **99문항** — 전부 개선
  방향(83 = 박스 마커가 자기 블록으로 분리[신명여중 SM1 이 이미 만드는 모양] · 13 = 숫자·문자
  객체화 · 3 = 기하 이름 로만화). 잃은 수식 0.
- 검증: 대진고 재렌더(해설 `rm OAB` 확인, 게이트 PASS) · 경산여고/경산고/성서고 재렌더
  `corpus_lint --xml` PASS + `<상자>`/`<보기>` 박스 육안 정상. 회귀 `test_content_parser` SO1
  (해설·본문 일치 · step 라벨 무회귀 · 중심 O 로만 · 비기하 A 이탤릭 유지).
  ⚠️ 기존 KB1 단언이 `A` 리터럴을 보고 있어 로만 랩(`\mathrm{A}`)으로 깨졌다 — 랩을 벗겨 비교
  하도록 고쳤다(**점 이름을 리터럴로 단언하는 테스트는 앞으로도 이렇게 깨진다**).
