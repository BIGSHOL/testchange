...
### ⚠️ 잔여 B형 (다음 차수 — 동부고/학남고 미마킹)
- **학남고 #15·16**: OCR text-타입 선택지에 마커 포함(`① 14`) + 폼 자동 ①②③ → 이중표기
  (렌더러 text-선택지 선행 submarker dedupe 필요).
- **학남고 #5**: figure 가 박스 마지막 블록 → 박스 그룹화가 그림노트를 박스 항목으로 흡수.
- **동부고 #17 B형**: 박스 뒤 발문 질문이 박스에 흡수(box overflow, SA2/`_trailing_question_split`
  미발동 케이스). 셋 다 추가 작업 필요 — 이 배치는 \limits·박스마커·A형까지 마무리.
