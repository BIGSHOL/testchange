...
`_COND_HEADER_RE` 박스머리로 오인(발문에서 `<보기>` 제거로 회피 — **파서 한계**, 추후 숙제),
② 쉼표목록을 한 수식(`2x^2, 7x, 6`)에 넣으면 `_split_comma_equations` 가 쉼표 제거(항별 분리로
회피). 둘 다 코드 회귀 아님. corpus `corpus/[상원중][1][25-1-기말] (원본)/`.
