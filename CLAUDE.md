...
### ⭐ 점 이름 로만 + 좌표 이탤릭 — HWP `rm` 은 뒤 전체로 번진다 (**진단 완료·구현 대기**)
...
- 고칠 곳: ① `latex_to_hwpeq` 에 `\mathit{…}`→`it {…}` 지원(현재 `\mathit` 는 **버려져** `it`
  미출력) ② `_romanize_point_names` 가 기하 문맥의 `점이름(좌표)` 을 `\mathrm{P}\mathit{(…)}`
  로 생성. **안전경계**: 확통 `P(X=r)`·`E(X)` 는 기하 키워드 없어 `_has_geometry_context`=False →
  이탤릭 유지(영향 없음). 함수 `f(x)`(소문자)도 대상 아님.

### ⭐ 변수/숫자 ↔ 단위 사이 백틱 얇은공백 누락 (**진단 완료·구현 대기**)
...
- 고칠 곳: 변환기 끝 후처리 — `<글자/숫자> rm <단위>` → `<글자/숫자> rm`<단위>`(숫자 케이스
  `5 rm`cm` 와 동일 형식). bare 와 달리 붙어있던 `5\mathrm{cm}` 류도 함께 정상화
  (`.testkit/unit_probe.py` 5케이스 렌더로 형태 확정).
